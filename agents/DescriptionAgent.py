...
class DescriptionAgent(Agent, ABC):
    ...
    def generate_llm_response(self,prompt):
        try:
            context_query = self.model.models.generate_content(model=self.model_id,contents=prompt)
            # Safely extract text output
            result_text = getattr(context_query, "text", "")
            return str(result_text).replace("sql", "").replace("", "").rstrip("\n")
        except Exception as e:
            print("error is generating in llm_response :{e}")
            return ""
