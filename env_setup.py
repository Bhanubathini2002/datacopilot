...
from utilities import ( BQ_REGION,LOGGING, VECTOR_STORE, PROJECT_ID, 
                       BQ_OPENDATAQNA_DATASET_NAME,EXAMPLES,FIRESTORE_REGION)
...
def create_firestore_db(
    ...
    firestore_database="opendataqna-session-logs",
):
    database_exists_cmd = [
    ...
    try:
        ...
        print("gcloud list timed out:", e)
        raise

    if database_exists_process.returncode == 0 and database_exists_process.stdout.strip():
        if database_exists_process.stdout.startswith(
            f"projects/{PROJECT_ID}/databases/{firestore_database}"
        ):
            print("Found existing Firestore database with this name already!")
            return
        else:
            print("Unexpected output:", database_exists_process.stdout)
            raise RuntimeError(
                "Issue with checking if the Firestore DB exists or not"
            )

    # If here, either returncode != 0 or no output. Log error before creating.
    if database_exists_process.stderr:
        print("gcloud list stderr:", database_exists_process.stderr)
...
def create_firestore_db(firestore_region=FIRESTORE_REGION,firestore_database="opendataqna-session-logs"):
    ...
    database_exists_process = subprocess.run(
        database_exists_cmd, capture_output=True, text=True , timeout=20
    )
...
def get_embeddings():
    ...
    #while current_dir != root_dir:
     #   for dirpath, dirnames, filenames in os.walk(current_dir):
      ...
      #          break  # Stop outer loop once found     mohith

    
      #  current_dir = os.path.dirname(current_dir)
      #  root_dir = current_dir

    dirpath = r"C:\New folder (3)\Open_Data_Qna\scripts"
    config_path = os.path.join(dirpath, 'data_source_list.csv')
    if os.path.exists(config_path):
...
if __name__ == '__main__':
    # create firestore database for storing embeddings
    create_firestore_db()
